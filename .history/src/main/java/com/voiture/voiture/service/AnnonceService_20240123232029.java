...
public class AnnonceService {
    ...
    public Annonce historiqueAnnonce(int idAnnonce, Utilisateur idUtilisateur) {
        return this.annonceRepository.findById(idAnnonce).map(
            annonce -> {
                List<Annonce> annoncesUtilisateur = annonceRepository.findAllByUtilisateurId(idUtilisateur.getId());
                // Faites quelque chose avec la liste d'annonces de l'utilisateur, si nécessaire
                return annonce;
            }
        ).orElseThrow(() -> new RuntimeException("Annonce inexistante"));
    }
    public Annonce historiqueAnnonce (Annonce annonce,Utilisateur idUtilisateur){
        return annonceRepository.save(annonce);
    }
