...
public class LieuService {
    ...
    public Lieu deleteLieu(int idLieu, Lieu modifier) {
        int idLieuToDelete = 1; // Remplacez 1 par l'ID réel du lieu que vous souhaitez supprimer
Lieu lieuModifier = new Lieu(); // Créez une instance de Lieu, vous n'avez besoin que de l'ID pour cette opération
        return this.lieuRepository.findById(idLieu).map(
            lieu -> {
                lieu.setNomLieu(modifier.getNomLieu());
